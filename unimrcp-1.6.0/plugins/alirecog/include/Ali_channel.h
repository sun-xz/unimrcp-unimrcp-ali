...
public:
	...
	void	Uninit();
	void	SetParams(apt_pair_arr_t* vendor_specific_params);
public:
	apt_bool_t	Start();
	apt_bool_t	Stop();
public:
...
private:
	void RecogMain();
	void RecogExit();
	void RecordStart(string& VoiceFilePath);
...
private:
	...
	static long				m_expireTime; // token的创建时间
	apt_bool_t			m_Runing;
	apt_bool_t			m_Starting;
	apt_bool_t			m_recogFlag;
	apt_bool_t			m_record;
	string				m_recordPath;
	apr_file_t* m_file;
