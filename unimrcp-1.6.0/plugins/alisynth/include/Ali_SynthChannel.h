...
public:
	...
	apt_bool_t	start(const string& voice_param,const string& speakText,Ali_synth_channel_t * engineCh);
	apt_bool_t	stop();
	apt_bool_t	uninit();
...
public:
	...
	void		readVoiceData(unsigned char *in_ptr,const apr_size_t size);
	void		readVoiceFrameData(unsigned char* in_ptr, const apr_size_t size);
public:
...
public:
	static void OnMetaInfo(NlsEvent* ev, void * arg);
...
private:
	...
	string		m_ParamVoiceName; // 发音人，参考阿里sdk说明,调用时动态传入
